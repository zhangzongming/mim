class BannerManager {
    ...
    renderUI() {
        ...
        let html = this.data.map((value) => {
            return ` <p><img src="${value.src}" alt=""></p>`
        }).join("");
        ...
        if (this.slider_nav) {
            ...
            //     console.log();
            // }
            let gtml = this.data.map(function(item) {
                return `<span><em></em><i class="ctrl-dot"></i><a href="#">${item.des}</a></span>`
            }).join('');
            this.slider_nav.append(gtml);
            ...
            this.oSpan.mouseenter(function () {
                ...
                $(this).addClass('active').children("em").css('opctiy','1');
